...
test("resolveDevWatchIgnoredPatterns returns root-scoped glob patterns", () => {
	const rootDir = path.resolve("H:/Programming/Web/FangYuan");
	const patterns = resolveDevWatchIgnoredPatterns(rootDir);

	assert.equal(patterns.includes(`${rootDir.replace(/\\/g, "/")}/docs/**`), true);
	assert.equal(patterns.includes(`${rootDir.replace(/\\/g, "/")}/tests/**`), true);
	assert.equal(patterns.includes(`${rootDir.replace(/\\/g, "/")}/src/**`), false);
});
