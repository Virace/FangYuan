...
- `develop`：开发集成分支。作为当前项目的默认日常开发基线，用于承接功能分支与修复分支的阶段性集成；默认不要求远端 Actions 持续通过，也不默认保证最终发布产物始终可用。
- 预发布分支：当前未固定命名。后续按发布需要创建，用于临时上线验证与发布前问题修复；验证通过后再同步到 `main`。
...
3. `develop` 主要承担日常集成职责；远端自动校验默认不以它为门禁，本地提交前验证优先在这里完成。
4. 若存在预发布分支，则从 `develop` 合入或择需补修到预发布分支，先做临时上线验证。
5. 预发布验证通过后再同步到 `main`；若当轮没有预发布分支，则由用户明确决定是否直接从 `develop` 收口到 `main`。

## 执行约束

- 非明确紧急场景下，不直接向 `main` 提交代码。
- 若存在多个并行子分支，统一先汇入 `develop`；正式发布时再根据当轮是否启用预发布分支决定后续同步路径。
- 与分支流转相关的说明、提交和 PR 讨论，默认以“`develop` 为日常集成分支，`main` 为正式分支，预发布分支按需启用”为前提。
...
- 常规功能 / 修复任务中，如果 `format` / `lint --write` / 其他自动修复命令带出了不属于本轮任务的文件变更，应先识别这些越界文件，默认将其单独提交，与本轮任务提交分离。
- 当任务明确要求全量质量门禁、全量自动修复，或明确要求“无关文件也一起修复并提交”时，自动修复带出的其他受检文件可纳入同一个维护提交；提交说明需表明这是质量门禁 / 格式化清理，并在最终汇报中列出实际影响范围。
...
- 主题运行所需且不提供用户自定义入口的固定图片资源放在 `src/theme-assets/`，并通过 Astro/Vite 资源管线导入。组件或布局需要这类资源时应直接 import 或经轻量 catalog 引用，不要写成外部站点 `assets/...` 路径。
- 仓库 demo 默认展示、可替换占位和可被外部配置覆盖的共享图片资源放在 `src/assets/`，并通过 Astro/Vite 资源管线导入。
...
- `public/` 不再作为默认静态资源发布入口；favicon、打赏二维码等默认走 `src/assets/` 或外部 `<siteRoot>/assets/`，公安备案图标等主题固定资源默认走 `src/theme-assets/`。
